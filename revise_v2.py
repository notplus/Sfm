...
def match_features(query, train):
    ...
    # flann_matches = flann.knnMatch(query,train,k=2)
    # matches = []
...
def main():
    ...
    K = config.K
    start_time=time.time()
    key_points_for_all, descriptor_for_all, colors_for_all = extract_features(img_names) # SIFT 特征计算
    end_time=time.time()
    print('time cost', end_time - start_time,'s')
    start_time=time.time()
    matches_for_all = match_all_features(descriptor_for_all) # 特征匹配 FLANN/BF
    end_time=time.time()
    print('time cost', end_time - start_time,'s')
    structure, correspond_struct_idx, colors, rotations, motions = init_structure(K, key_points_for_all, colors_for_all, matches_for_all)   #初始化点云结构
    ...
    for i in range(1, len(matches_for_all)):
        ...
        _, r, T, _ = cv2.solvePnPRansac(object_points, image_points, K, np.array([])) # 求解变换矩阵
        ...
        next_structure = reconstruct(K, rotations[i], motions[i], R, T, p1, p2) # 三维重建
